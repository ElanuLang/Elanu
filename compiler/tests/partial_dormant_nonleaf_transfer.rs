...
#[test]
fn transfer_dormant_nonleaf_changes_only_parent_provenance_edge() {
    ...
    assert_eq!(
        backing_before.len(),
        4,
        "seed should produce source, destination, child Folder, and descendant Document backing"
    );
    ...
    assert_eq!(
        ...
        2,
        "child Folder and descendant Document should remain dormant"
    );

    runtime
        .run_action("transferChild")
        .expect("nonleaf transfer should use exact provenance metadata without member loading");

    let mut provider = runtime.into_provider();
    assert_eq!(
        provider.loads.len(),
        2,
        "nonleaf provenance transfer must not read child or descendant backing"
    );
    assert_eq!(
        provider.backing, backing_before,
        "provenance-only transfer must not mutate any modeled member backing"
    );
    assert_eq!(
        provider.replacements.last(),
        Some(&Vec::<Vec<u8>>::new()),
        "provenance-only transfer should publish only manifest metadata"
    );

    provider.loads.clear();
    provider.replacements.clear();
    let mut restarted = PartialPersistentRuntime::open(checked, provider)
        .expect("transferred nonleaf world should restart without dynamic backing reads");
    assert_eq!(restarted.dormant_backing_keys().len(), 4);

    restarted
        .run_action("proveDestinationOwnsChild")
        .expect("new owner should prove transferred child provenance while child remains dormant");
    restarted
    ...
    assert!(
        ...
        "provenance proofs after restart must not materialize child or descendant backing"
    );
    let mut restarted = PartialPersistentRuntime::open(
        check_source_with_runtime_models(SOURCE).expect("source should still check"),
        provider,
    )
    .expect("provenance proof actions should leave world restartable");

    restarted
        .materialize_designation("selectedChild")
        .expect("transferred child should remain independently materializable");
    restarted
