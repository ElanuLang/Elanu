...
Path("compiler/src/runtime/undo_history_negative_evidence.rs").write_text(r'''use super::persistence::{PersistenceImage, PersistenceProvider, PersistentRuntime};
...
fn opaque_provider_history_restores_prior_committed_world_without_inverse_logic() {
    ...
    assert_eq!(
        app.value("selectedInSource").unwrap(),
        Value::Bool(false)
    );
